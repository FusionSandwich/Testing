...
def phase1LinePoint (t : ℝ) : Point3 := !₂[t, 0, 0]
...
theorem phase1LinePoint_dist (a b : ℝ) :
  ...
  simp [phase1LinePoint, dist_eq_norm_sub, PiLp.norm_eq_of_L2]
...
def f3 (n : ℕ) : ℕ :=
  Nat.findGreatest (AttainableContactCount n) (n.choose 2)

/-- The defining upper bound for `f3`. -/
theorem f3_le_choose (n : ℕ) : f3 n ≤ n.choose 2 :=
...
theorem exists_contact_extremizer (n : ℕ) :
  ...
  have hnonempty := attainableContactCount_nonempty n
