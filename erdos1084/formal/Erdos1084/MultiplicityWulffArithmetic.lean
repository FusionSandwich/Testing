...
theorem multiplicity_twoLevel_cube_bound (x y : ℝ) :
  ...
  exact mul_nonneg
    ...
    (multiplicity_quadraticFactor_nonneg x y)
...
theorem multiplicity_totalRoot_square_le
  ...
  have hcube : (totalRoot ^ 2) ^ 3 ≤ sumSq ^ 3 := by
    calc
      ...
      _ = (s.sum fun i => root i ^ 3) ^ 2 := by rw [htotal]
      _ ≤ (s.sum fun i => root i ^ 2) ^ 3 :=
...
theorem multiplicity_wulff_energy_assembly
  ...
  have hlevels : C * (s.sum fun i => root i ^ 2) ≤ s.sum energy := by
    calc
      C * (s.sum fun i => root i ^ 2) =
          s.sum (fun i => C * root i ^ 2) := by
        rw [Finset.mul_sum]
      _ ≤ s.sum energy := by
