...
/-!
# Concrete degree-eleven endpoint algebra

This module discharges every algebraic and exact-inequality input in the concrete
one-radius monotonicity proof.  It proves the exact profile, its candidate derivative,
the uniform negative-derivative inequality, and the exact crossing at `kpRadius`.

The final calculus wrapper turning the negative derivative into `StrictAntiOn` is kept in
the ordinary proof dossier while its Lean API normalization is completed.
-/

noncomputable section

/-- Cosine of `ρ₁₁-π/6`, written in endpoint coordinates. -/
def kpEndpointElevenCos : ℝ :=
  (rtLower * rtS + kpLowerY) / 2

/-- Sine of `ρ₁₁-π/6`, written in endpoint coordinates. -/
...
/-- The concrete derivative expression is negative throughout `[2,r_*]`. -/
...
/-- Exact crossing of the concrete degree-one and degree-eleven profiles. -/
