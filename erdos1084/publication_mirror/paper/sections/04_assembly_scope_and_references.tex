...
\section{A matching-order FCC construction}
\label{sec:lower}

The surface exponent in \cref{thm:main} is optimal in order.  In the original
minimum-distance-one normalization, let
\[
 u_1=\frac{(0,1,1)}{\sqrt2},
 \qquad
 u_2=\frac{(1,0,1)}{\sqrt2},
 \qquad
 u_3=\frac{(1,1,0)}{\sqrt2}.
\]
These generate the FCC lattice scaled to minimum distance one.  For positive
integers $a,b,c$, put
\[
 X_{a,b,c}
 =\{iu_1+ju_2+ku_3:
 0\le i<a,\ 0\le j<b,\ 0\le k<c\}.
\]
Its $abc$ points have exactly
\begin{equation}\label{eq:lowerbox}
 E(a,b,c)
 =6abc-3ab-3ac-3bc+a+b+c
\end{equation}
unit-distance pairs.  Indeed, orient the contacts in the six directions
\[
 u_1,u_2,u_3,u_1-u_2,u_1-u_3,u_2-u_3.
\]
The corresponding edge counts are
\[
 (a-1)bc,\quad a(b-1)c,\quad ab(c-1)
\]
and
\[
 (a-1)(b-1)c,\quad(a-1)b(c-1),\quad a(b-1)(c-1),
\]
whose sum is \cref{eq:lowerbox}.

For $a=b=c=m$, this gives the exact subsequence
\begin{equation}\label{eq:lowercube}
 \boxed{
 f_3(m^3)\ge6m^3-9m^2+3m.}
\end{equation}
Thus the deficit is at most $9m^2-3m$ along cubic cardinalities.

There is also an elementary all-cardinality estimate.  Let
$m=\lfloor n^{1/3}\rfloor$ and $t=n-m^3$.  Append to $X_{m,m,m}$ the $t$
consecutive FCC points
\[
 mu_1,(m+1)u_1,\ldots,(m+t-1)u_1.
\]
They continue a contact chain from $(m-1)u_1$, so they create at least $t$ new
contacts.  Since $t<3m^2+3m+1$,
\[
 6n-f_3(n)
 \le9m^2-3m+5t
 <24m^2+12m+5.
...
\begin{equation}\label{eq:loweralln}
 \boxed{
 f_3(n)>6n-24n^{2/3}-12n^{1/3}-5.}
\end{equation}
The constants in \cref{eq:loweralln} are not optimized.  The construction proves
that $n^{2/3}$ is the correct order of the secondary term, but it does not identify
the exact asymptotic coefficient.

\section{Verification and scope}

The finite upper theorem uses the following named published inputs:
...
The lower construction in \cref{sec:lower} is elementary and independent of the
four upper-bound inputs.
...
completion proof, the FCC lower-count construction, or foundational versions of
(i)--(iv); ordinary theorem parameters remain visible in the load-bearing
signatures.
