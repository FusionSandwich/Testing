...
azimuthal rates uniquely.  The displayed differences are the exact square-grid
polar coordinate differences after cancelling the nonzero `sin h` or `cos h`
factor. -/
theorem squarePolar_rates_forced
    (s aMer aLeft aRight : ℝ)
    (hs : s ≠ 0)
    (htransverse : aRight = aLeft)
  ...
  have hleft : aLeft = 1 / (4 * s ^ 4) := by
    rw [htransverse, hmer] at htangent
    field_simp [hs] at htangent ⊢
    nlinarith
  exact ⟨hmer, hleft, htransverse.trans hleft⟩
...
theorem squarePolar_totalRate_forced
  ...
  obtain ⟨hmer, hleft, hright⟩ :=
    squarePolar_rates_forced
      s aMer aLeft aRight hs htransverse hradial htangent
