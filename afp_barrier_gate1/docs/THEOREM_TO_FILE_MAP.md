...
| 1.4 scaling and converse | Theorem 1.2 | `LocalSphericalFeasibility.lean`, `ExactLocalRows.lean` | all local exact rows |
...
| 2.4 mixed parameterization | Theorem 2.2 | both directions in `AntipodalFeasibility.lean` | boundary/interior cases |
...
| 3.2 constructive bound | Theorem 3.1 | finite barycentric construction in proof | triangle/square margins |
| 3.3 rate/coefficient bounds | Theorem 3.3 | exact identity in `ExactLocalRows.lean`; weighted bounds in `QuantitativeExactLocal.lean`; loss specialization in `QuasiUniformLossBounds.lean` | equilateral rate |
...
| 4.5 LP duality/complementarity | Theorem 4.3 | `DualCertificate.lean`; precise standard transfer | sign audit |
| 4.6 local-not-global | Theorem 4.4 | exact `Fin` proof in `SharedEdgeEquilibrium.lean` | four-cycle certificate |
| 4.7 averaging reconciliation | Theorem 4.5 | abstract linear averaging/reconciliation core in `GroupAveraging.lean`; geometric equivariance and edge-orbit propagation proved in prose | equal-mass square |
| 4.8 global perturbation | Theorem 4.6 | pseudoinverse/range transfer | explicit radius |
| 5 examples and claim control | Section 5 | exact finite examples where practical | `exact_local_global_audit.py` |
