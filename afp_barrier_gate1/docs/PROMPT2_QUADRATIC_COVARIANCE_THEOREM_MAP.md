...
| Shifted trace-free form condition | Equation (2.9) | Ordinary finite linear algebra | Platonic second-moment check excludes nonzero constant samples | PROVED |
| Sampling map and kernel | Section 3 | Ordinary linear definitions | Exact evaluation matrices | PROVED |
| Genuine sampled exact space | Theorem 3.1, equation (3.3) | Ordinary restriction-of-linear-map proof | Sampled image computed exactly | PROVED |
| Dimension identity with intersection | Equation (3.4) | Standard finite rank-nullity applied to `S_X|E_form` | Negative test rejects naive kernel subtraction | PROVED / exact regression |
...
| Bounded additive resonance search | Section 8.4 | Pell-type reformulation in ordinary text | Exact bounded list | COMPUTATIONAL / arithmetic context |
| General spectral-product hierarchy | Section 8 conclusion | Kill-criterion analysis | No new sampled dimension tradeoff or global result | REJECTED FOR PROMPT 2 |
...
The project does not reprove general finite-dimensional rank-nullity, equality
in Jensen's inequality, finite continuous-time Markov uniformization,
spherical-harmonic Fischer decomposition, or basic real representation
irreducibility facts. The paper states the exact hypotheses under which those
standard inputs are used. The covariance and sampling consequences are
transferred to the concrete finite generator, embedding, matrices, and signs.
