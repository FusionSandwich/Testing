...
Fix an integer \(N\ge2\). Let \(\mathcal G_N\) be the unreduced
cell-centred latitude--longitude graph
...
The verified baseline theorems `equalAngleGrid_coordinate_exact`,
`equalAngleGrid_meridional_shared`, the north/south endpoint theorems, the
actual-edge positivity theorems, and
`equalAngleGrid_squareRingRate_le_polar` give a positive reversible
shared-conductance construction on precisely this support. It attains the row
in (2.1), and its maximum rate occurs on the polar rings. Therefore
...
Thus the graph-class obstruction is sharp at every grid order \(N\ge2\), and
