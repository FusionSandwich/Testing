...
\(f=x_1+x_2\). The resulting square graph is positive, symmetric,
conservative, and connected. Direct evaluation gives
...
Each coordinate character has eigenvalue \(-2\), their product has eigenvalue
\(-4\), and the two possible flips contribute squared increments \(4+4\),
with the factor \(1/2\) in \(\Gamma\).

Any universal positive-generator claim forbidding a nonzero centered doubled
square is therefore false.

## Semigroup variance identity

Let \(P_t=e^{tL}\). Since \(L\mathbf1=0\), one has
\(P_t\mathbf1=\mathbf1\). If \(Lf=-\lambda f\) and
\(L(f^2-c)=-2\lambda(f^2-c)\), matrix-exponential calculus gives

\[
 P_tf=e^{-\lambda t}f,\qquad
 P_t(f^2)=c\mathbf1+e^{-2\lambda t}(f^2-c\mathbf1).
\]

Therefore

\[
 \boxed{P_t(f^2)-(P_tf)^2=c(1-e^{-2\lambda t}).} \tag{3.1}
\]

Conversely, if \(Lf=-\lambda f\) and (3.1) holds near zero, differentiating at
zero gives \(L(f^2)-2fLf=2\lambda c\). The product identity identifies the left
side with \(2\Gamma(f)\), so \(\Gamma(f)=\lambda c\), exactly the square
resonance criterion. This semigroup argument uses no positivity.

## Jensen variance and its equality case

Now separately assume a positive generator. Each row
\(p_j=P_t(i,j)\) is a probability vector, and, for
\(m=\sum_jp_jf_j\),

\[
 P_t(f^2)(i)-(P_tf(i))^2
 =\sum_jp_j(f_j-m)^2\ge0. \tag{3.2}
\]

Equality holds exactly when \(f_j=m\) for every \(j\) with
\(P_t(i,j)>0\). For a finite irreducible chain and \(t>0\), choose \(q\)
strictly larger than every exit rate and set \(K=I+L/q\). Then \(K\) is
stochastic with positive diagonal and

\[
 e^{tL}=e^{-qt}\sum_{n\ge0}\frac{(qt)^n}{n!}K^n.
\]

An irreducible positive-probability \(K\)-path from \(i\) to \(j\) makes one
Poisson term positive, so every \(P_t(i,j)>0\). Jensen equality at one state
then forces global constancy. This Markov-kernel equality and uniformization
input is EXTERNAL. It is not square resonance: the Boolean example has positive
variance \(2(1-e^{-4t})\) while its centered square is exactly resonant.
...
\(x^2-2y^2=-1\) plus \(x\equiv1\pmod4\). The standard negative-Pell
classification says every positive solution is
\((1+\sqrt2)^{2n+1}\). Multiplication by \(3+2\sqrt2\) sends \(x\) to
\(3x+4y\equiv-x\pmod4\), so the parity class alternates from the initial
\(x=1\). Hence \(x\equiv1\pmod4\) exactly when \(n\) is even, and all
admissible solutions are

\[
 x+y\sqrt2=(1+\sqrt2)^{4m+1},\qquad m\ge0.
\]
...
\[
 \ell'=17\ell+12J+14,\qquad J'=24\ell+17J+20.
\]

Completeness of the positive negative-Pell classification and the
Clebsch--Gordan decomposition are EXTERNAL standard inputs; the parity transfer
and recurrence are proved explicitly. This is an exact
\(\ell\)-indexed component-availability classification, not a claim that one
product isolates the component and not a positivity no-go.
...
| sample | rank \(E_2\) | rank \(E_4\) | rank \([E_2\ E_4]\) | relation |
|---|---:|---:|---:|---|
| tetrahedron | 3 | 4 | 4 | \(V_2\subset V_4\) |
| octahedron | 2 | 3 | 3 | \(V_2\subset V_4\) |
| cube | 3 | 4 | 4 | \(V_2\subset V_4\) |
| icosahedron | 5 | 5 | 5 | \(V_2=V_4\) |
| dodecahedron | 5 | 9 | 9 | \(V_2\subset V_4\) |

These concatenated ranks are exact, not floating-point threshold decisions.
Concrete aliases are checked as well. In the common-radius raw coordinates
used by the audit, let

\[
 P=xy(x^2+y^2-6z^2),\quad Q=xy(x^2-y^2),\quad
 D=(x^2-y^2)(x^2+y^2-6z^2),
\]

all harmonic of degree four, and let \(\varphi=(1+\sqrt5)/2\). Then
\(-P/4=xy\) on the tetrahedron and cube, \(D=x^2-y^2\) on the octahedron,
\(P/(\varphi+2)=xy\) on the icosahedron, and

\[
 -P/4-\frac7{4\sqrt5}Q=xy
\]

on the dodecahedron. Normalizing a common-radius set only rescales the
degree-four side by the common squared radius and does not change the sampled
subspace inclusion. Consequently no operator, even signed, can assign the
distinct continuous \(H_2\) and \(H_4\) eigenvalues simultaneously on these
aliases.
