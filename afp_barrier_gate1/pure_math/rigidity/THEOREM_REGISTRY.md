...
| P3-S | near-equality and round triangulation hypotheses plus explicit side-box inequalities | common valence, Platonic type, edge-length distance to `theta_q` | P3-4–11, positive Gram/Heron determinant, explicit angle derivatives, integer gap | domain margins are mandatory | §6.1–6.4 | selected finite inequalities | zero-defect and positive-width angle/valence audit |
| P3-S* | P3-S reference loss satisfies the explicit margins in §6.5 | single threshold `eta_*=kappa delta_*^2` suffices | logarithmic bounds and fixed Gram/Heron-certified box | no uniform threshold at geometric degeneracy | §6.5 | ordinary proof | closed-threshold stress at q=3,4,5 |
...
| P3-CB | exact antipodal equality `ell=2`, `r=1` | `C_i=4 Omega_i Omega_i^T`; tangent coefficient zero and no unit tangent normalization | direct displacement algebra | excluded from P3-C2 | §7.4 | `qOne_antipodal_*` | exact two-state regression |
| P3-C4 | positive weighted octahedral family | `L Omega=-2Omega`, `r=2`, `ell=1`, `Q=1`, axial iff all `g` equal | exact conductance computation | none within stated family | §7.5 | selected scalar identities | exact SymPy |
| P3-C5 | P3-C4 and genuine sampling map | degree-two sampled exact space is `{0}` | exact positive determinant of `P+2I` and stochastic contraction | form aliases retained | §7.5 | ordinary finite proof | exact determinant and coordinate audit |
| P3-C6 | `1<=Q_i<=1+eta` | radial error in `[0,4eta/r_i]`; no tangent bound | P3-C1 | anisotropy tends to `1/2` at `eta=0` | §7.6 | finite scalar inequality | exact parameter limit |
...
s_A0      = 2 sqrt(m_S m_T^3), from the Gram/Heron determinant floor
