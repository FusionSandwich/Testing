...
| quadrature-generator co-design | all-orders frontier input | **P2C proved scoped method core** | lift overlap | exact fixed-candidate inner optimization, protected outer existence/stationarity, certified graph/adaptive updates, rotation separation and all-orders rate | no nonconvex global optimum, arbitrary-design shared lift, whole-sequence convergence, continuum response convergence or streaming-ray cure | II |
...
| positive generator lifts of designs | possible appendix | design input | foundation | nonempty balanced polytope and shared conductances | one-way design implication alone does not earn Paper III | III |
| extreme lift-cone structure | none | optimization context | central target | result beyond generic `Ax=b,x>=0` | generic LP face facts must not be branded novel | III |
| global minimum negativity dual | optional signed appendix | certificate tool | central target | exact primal-dual theorem, attainment, values/families | one local negativity bound is insufficient | III |
| residual-negativity Pareto theorem | frontier context | design tradeoff | flagship target | exact frontier and equality families | one signed example is insufficient | III |

Paper I is earned only by the sharp sampled-fidelity frontier, equality,
stability, and a matching positive family.  Paper II is earned only by lower
physical response error or lower cost at equal response error while retaining
conservation and monotonicity.  Paper III survives only if lift-cone geometry,
global negativity, and the Pareto theorem are genuinely non-generic; otherwise
its strongest results merge into Paper I.
...
## P2C quadrature–generator co-design boundary refinement

| Result / deliverable | Paper I role | Paper II role | Paper III role | Prerequisite / novelty threshold | Kill criterion and prohibited overclaim | Source owner |
|---|---|---|---|---|---|---|
| raw moving-Gram shell epigraph | supplies sampled quotient | proved P2C moving-kernel theorem | none | positive masses, fixed candidate and full-output Gram inequality | no moving-rank SVD shortcut, coefficient norm, or output compression | I/II |
| exact inner / protected outer problem | supplies sharp lower and incumbent upper bounds | proved global fixed-candidate SDP plus compact protected outer existence | lift-cone context | P2A certificates; compact parameter set; continuous feasible correspondence; explicit rank/Robinson and optimizer-face gates | no global optimum for nonconvex nodes/graphs and no reduced-value continuity from nonunique optimality alone | II/III |
| orbit, Riemannian and alternating strategies | none | proved feasibility/descent/stationary-subsequence results under displayed regularity gates | none | exact proximal or restored Armijo steps, prox-regularity/constant rank, gradient-relatedness, tangent coverage or joint safeguard | no whole-sequence convergence, uncertified Clarke stationarity, or promotion of finite-pool behavior to the continuous theorem | II |
| certified graph updates | none | proved edge-addition monotonicity, conditional deletion and finite no-revisit termination | support-face context | complete inner reoptimization, rebuilt feasibility and objective enclosures | no minimum-cardinality, arbitrary-neighborhood stationarity or global graph optimum | II/III |
| direct design and AB orbit adapter | P1E incumbent | proved dense centered initializer; audited generic icosahedral-orbit admission path | possible lift examples | positive centered masses; declared orbit representatives, masses and harmonic exactness verification | spherical-design strength or local feasibility alone does not imply a sparse shared generator; built-in fixture is not a published AB table | I/II/III |
| adaptive response refinement | none | exact enriched-discrete primal–adjoint identity | none | declared enriched discrete reference and adjoint residual | no continuum or physical-model convergence without a separate theorem/reference hierarchy | II |
| all-orders co-designed rate | imports P1B/P1E constants | proved `Theta(h^2)` inner-optimal family | none | accepted reflected-ring witness and fixed rate cap `64 pi^2 h^-2` | finite fitted slopes and practical reduced-constant meshes are regressions only | I/II |
| rotation and streaming scope | none | proved collision/common-rotation identities and certified rotation-net enclosure; interpolation audited separately | none | intrinsic graph covariance, equivariant application data, and covering-radius/Lipschitz remainder where continuum extrema are claimed | no inference from collision `D2` to streaming rays, and no finite rotation sample presented as a continuum supremum | II |
| equal-cost physical response improvement | none | still required in P2E | none | frozen matched-node/cost benchmark hierarchy and six-channel error ledger | P2C completes the co-design method, not the Paper-II empirical go/no-go gate | II |
