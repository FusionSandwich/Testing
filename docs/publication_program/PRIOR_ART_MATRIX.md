...
| M11 — [Bienvenue--Naceur--Carrier--Hebert, flexible moment-preserving monotone angular Fokker--Planck discretization](https://doi.org/10.1080/00295639.2025.2462891) | Fixed nonorthogonal spherical quadratures; Voronoi-neighbor shared symmetric coefficients; a pseudoinverse moment system; positivity/exactness reported for tested product, level-symmetric and Lebedev sets, with general characterization left for further study. | Their shared-coefficient generator matches `gamma/w` after sign conversion. The accessible deposited manuscript's first-shell target and its printed moment equation differ by a factor two; P2A uses the explicit `L Omega=-2Omega` convention and transfers no factor silently. | Direct prior art for shared moment-preserving angular generators and the linear moment system. It does not give arbitrary-graph positive feasibility, alias-correct `H_2` optimization, the Paper-I rate frontier, or solver-independent primal/dual/Farkas certificates. | HYPOTHESES_AND_NORMALIZATION_AUDITED / II |
...
## P2A convex-design transfer audit

| Source | Exact external content used | Transfer into P2A | Precise non-transfer boundary |
|---|---|---|---|
| Bienvenue--Naceur--Carrier--Hebert | shared coefficients on fixed quadratures and a moment-system pseudoinverse tested on named quadrature families | establishes direct angular-discretization precedent; signs and the factor-two discrepancy are audited line by line | no universal positivity/feasibility theorem, no sampled quotient objective, no rate-defect frontier and no conic certificate package |
